...
public class EmiratesBooking  extends CommonTestActions {
	...
	public void enterBookingDetails(String from,String to,String fromdate,String returnDate,int adults,int child,int infants) {
		typeAndSelect(input_departure, from, "select value"); 
		typeAndSelect(input_arrival,to, "select value"); 
		click("//td[@data-string='"+fromdate+"']/button","Select from Date");
		...
		for(int i=1;i<adults;i++) click(passenger.replace("XXX","adults"),"Adults");
		for(int i=0;i<child;i++) click(passenger.replace("XXX","children"),"Children");
		for(int i=0;i<infants;i++) click(passenger.replace("XXX","infants"),"Infants");
